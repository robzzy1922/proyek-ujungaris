<aside class="w-64 bg-white shadow-md flex flex-col justify-between">
    <div>
        <div class="flex items-center px-6 py-4 border-b">
            ...
            <div>
                <h1 class="text-lg font-bold leading-tight">KUWU PANEL</h1>
                <p class="text-sm text-gray-500">Desa Ujungaris</p>
            </div>
        </div>
        <nav class="mt-6 space-y-2 px-4">
            <!-- Dashboard -->
            <a href="{{ route('kuwu.dashboard') }}" class="flex items-center px-3 py-2 bg-blue-100 text-blue-600 rounded-lg font-medium">
            ...
            <a href="{{ route('kuwu.riwayat') }}" class="flex items-center px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-lg">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- Logout -->
    <div class="px-4 py-4">
        <form method="POST" action="{{ route('kuwu.logout') }}">
            @csrf
